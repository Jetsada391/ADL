...
if (empty($y)) {
  $sql_cmd = "SELECT * FROM `report`ORDER BY ID_year ASC ";
} else if (empty($y->Moo)){
  $sql_cmd =   "SELECT * FROM `report` WHERE  ID_year BETWEEN $y->start_year AND $y->end_year  ORDER BY ID_year  ,CAST(Moo as int) ASC";

}else if (empty($y->adl)){
  $sql_cmd =   "SELECT * FROM `report` WHERE Moo = '$y->Moo' AND ID_year BETWEEN $y->start_year AND $y->end_year  ORDER BY ID_year ,Moo ASC";

}
else{
  $sql_cmd =   "SELECT * FROM `report` WHERE Moo LIKE $y->Moo   AND Resualt_ADL LIKE '$y->adl'
