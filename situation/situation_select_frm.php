...
if(empty($y)){
  $sql_cmd = "SELECT situation.IDCard, situation.ID_Year,situation.SituID,situname.Situ_Name 
              FROM `situation` 
              INNER JOIN situname ON situation.SituID=situname.situID;";

}else{
  
$sql_cmd = "SELECT situation.IDCard, situation.ID_Year,situname.Situ_Name 
            FROM `situation` 
            INNER JOIN situname ON situation.SituID=situname.SituID
            WHERE IDCard LIKE '%{$y->nah}%' or
                  ID_Year LIKE '%{$y->nah}%' or 
                  situation.SituID LIKE '%{$y->nah}%' or 
                  situname.Situ_Name LIKE '%{$y->nah}%'";
